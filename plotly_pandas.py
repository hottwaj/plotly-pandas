...
def charts_table(charts, cols):
    ...
    table_content = """
<style>
/* for chart subplots tables produced by 'charts_table' */
.table-no-border {
    ...
    for row in grouped(charts, cols):
        ...
        for chart in row:
            if chart is not None:
                if isinstance(chart, PlotlyChartBundle):
...
def scatter(df, x_col, y_col, 
    ...
    bundle = PlotlyChartBundle(data_layout,
                               width = width,
                               height = height,
...
def chart(dataframe, layout = dict(), column_settings = dict(), all_columns_settings = dict(), x_and_y = True, 
    ...
    """Generic plot from data in a DataFrame.  Can be used for e.g. lines, bars, pies and histograms.
    ...
    return PlotlyChartBundle(data_layout,
                             width = width,
                             height = height,
                             config = default_config)
